...
const Table: React.FC = () => {
  ...
  useEffect(() => {
    fetch('table3-data.json') // Change this path according to your project structure
  ...
  const totalxyzPercentage = data.reduce((acc, person) => acc + parseFloat(person.xyz.toString().replace('%', '')), 0);

  return (
    <div style={{margin:'40px'}}>
      <table style={{ height:'200px', width: '100%', borderCollapse: 'collapse', borderSpacing: 0, backgroundColor: 'black', borderRadius: '10px', margin: '10px' }}>
        <thead>
          <tr>
            <th style={tableHeaderStyle}>PRODUCT</th>
            <th style={tableHeaderStyle}>Q1-23</th>
            <th style={tableHeaderStyle}>Q2-23</th>
            <th style={tableHeaderStyle}>Q3-23</th>
          </tr>
        </thead>
        <tbody>
          {data.map(person => (
            <tr key={person.id}>
              ...
              <td style={tableCellStyle}>{person.xyz}</td>
            </tr>
          ))}
          <tr>
            ...
            <td style={tableCellStyle}>{totalxyzPercentage.toFixed(2)}%</td>
          </tr>
        </tbody>
      </table>
    </div>
